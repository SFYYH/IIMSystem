...
public class SystemController {
	...
	@RequestMapping(value="/login",method=RequestMethod.POST)
	@ResponseBody
	public Result<Boolean> login(HttpServletRequest request,String username,String password,String cpacha,int type){
		if(username == null){
			return Result.error(CodeMsg.ADMIN_USERNAME_EMPTY);
		}
		if(username.length() < 4){
			return Result.error(CodeMsg.USERNAME_MIN_LENGTH_ERROR);
		}
		if(username.length() > 18){
			return Result.error(CodeMsg.USERNAME_MAX_LENGTH_ERROR);
		}
		if(password == null){
		...
		if(type == LoginType.ADMINISTRATOR.getCode()){
		...
		else if(type == LoginType.JUDGE.getCode()){
			//表示验证码正确，开始查询数据库，检验密码是否正确
			Judge byMobile = judgeService.findByMobile(username);
			//判断是否为空
			if(byMobile == null){
				return Result.error(CodeMsg.ADMIN_USERNAME_NO_EXIST);
			}
			...
			//表示密码正确，接下来判断用户状态是否可用
			if(byMobile.getStatus() == Judge.JUDGE_STATUS_UNABLE){
				return Result.error(CodeMsg.ADMIN_USER_UNABLE);
			}
			//检查用户所属角色状态是否可用
			...
			//检查一切符合，可以登录，将用户信息存放至session
			request.getSession().setAttribute(SessionConstant.SESSION_JUDGE_LOGIN_KEY, byMobile);
			request.getSession().setAttribute("type",type);
			//销毁session中的验证码
			request.getSession().setAttribute("admin_login", null);
			//将登陆记录写入日志库
			operaterLogService.add("评委【"+byMobile.getName()+"】于【" + StringUtil.getFormatterDate(new Date(), "yyyy-MM-dd HH:mm:ss") + "】登录系统！");
			log.info("评委成功登录，judge = " + byMobile);
		}else if(type == LoginType.STUDENT.getCode()){
			//表示验证码正确，开始查询数据库，检验密码是否正确
			Student byPhone = studentService.findByStudentNumber(username);
			//判断是否为空
			if(byPhone == null){
				return Result.error(CodeMsg.ADMIN_USERNAME_NO_EXIST);
			}
			...
			//表示密码正确，接下来判断用户状态是否可用
			if(byPhone.getStatus() == Student.NORMAL){
				return Result.error(CodeMsg.ADMIN_USER_UNABLE);
			}
			//检查用户所属角色状态是否可用
			if(byPhone.getRole() == null || byPhone.getRole().getStatus() == Role.ADMIN_ROLE_STATUS_UNABLE){
				return Result.error(CodeMsg.ADMIN_USER_ROLE_UNABLE);
			}
			//检查用户所属角色的权限是否存在
			if(byPhone.getRole().getAuthorities() == null || byPhone.getRole().getAuthorities().size() == 0){
				return Result.error(CodeMsg.ADMIN_USER_ROLE_AUTHORITES_EMPTY);
			}
			//检查一切符合，可以登录，将用户信息存放至session
			request.getSession().setAttribute(SessionConstant.SESSION_STUDENT_LOGIN_KEY, byPhone);
			request.getSession().setAttribute("type",type);
			//销毁session中的验证码
			request.getSession().setAttribute("admin_login", null);
			//将登陆记录写入日志库
			operaterLogService.add("学生【"+byPhone.getName()+"】于【" + StringUtil.getFormatterDate(new Date(), "yyyy-MM-dd HH:mm:ss") + "】登录系统！");
			log.info("学生成功登录，student = " + byPhone);
		}
	...
	public String logout(){
		...
		if(type == LoginType.ADMINISTRATOR.getCode()){
			...
			if(loginedJudge != null){
				SessionUtil.set(SessionConstant.SESSION_JUDGE_LOGIN_KEY,null);
			}
		}else if(type == LoginType.STUDENT.getCode()){
	...
	public Result<Boolean> updatePwd(@RequestParam(name="oldPwd",required=true)String oldPwd,
		...
		if(type == LoginType.ADMINISTRATOR.getCode()){
			...
			Judge loginedJudge = SessionUtil.getLoginedJudge();
			if(!loginedJudge.getPassword().equals(oldPwd)){
				return Result.error(CodeMsg.ADMIN_USER_UPDATE_PWD_ERROR);
			}
			if(StringUtils.isEmpty(newPwd)){
				return Result.error(CodeMsg.ADMIN_USER_UPDATE_PWD_EMPTY);
			...
			//保存数据库
			judgeService.save(loginedJudge);
			//更新session
			loginedJudge.setPassword(newPwd);
			SessionUtil.set(SessionConstant.SESSION_JUDGE_LOGIN_KEY, null);
		}else if(type == LoginType.STUDENT.getCode()){
			...
			//保存数据库
			studentService.save(loginedStudent);
			//更新session
			loginedStudent.setPassword(newPwd);
			SessionUtil.set(SessionConstant.SESSION_STUDENT_LOGIN_KEY, null);
		}
